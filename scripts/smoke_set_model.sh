...
if [[ -n "${PIMIENTO_OMP_BIN:-}" ]]; then
  OMP_BIN="$PIMIENTO_OMP_BIN"
elif OMP_BIN="$(command -v omp 2>/dev/null)"; then
  :
else
  echo "smoke_set_model: omp not found — set PIMIENTO_OMP_BIN or put omp on PATH" >&2
  exit 1
fi
export PIMIENTO_OMP_BIN="$OMP_BIN"
...
omp = os.environ.get("PIMIENTO_OMP_BIN")
if not omp:
    raise SystemExit("PIMIENTO_OMP_BIN not set")
