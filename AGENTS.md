...
Cloud Agents run on headless Linux (Ubuntu 24.04, x86_64). `scripts/cloud-agent-setup.sh` is the idempotent bootstrap (GPUI/Zed system libs, stable + nightly toolchains, `cargo-nextest`, the real `omp` harness + Cursor auth + model config, `cargo fetch`); it is the environment `install` command and is safe to re-run.

- **Real `omp`, authenticated, cheapest model — required for every agent.** The setup installs `omp` to `~/.local/bin`, writes `~/.omp/agent/config.yml` pinning every role to `cursor/composer-2.5` (Cursor's cheapest model), and installs `~/.local/share/omp-cursor-auth.sh` (sourced from `~/.profile` + `~/.bashrc`). omp's `cursor` provider needs a short-lived session JWT, **not** the raw `CURSOR_API_KEY`; the helper exchanges the injected `CURSOR_API_KEY` runtime secret for a token via `POST https://api2.cursor.sh/auth/exchange_user_api_key` and exports `CURSOR_ACCESS_TOKEN` (cached ~45 min). Because Pimiento discovers omp through `$SHELL -lc`, the login shell both finds omp and mints the token, so auto-connect uses the real omp with `composer-2.5`. Keep `CURSOR_API_KEY` as the secret — a `CURSOR_ACCESS_TOKEN` secret would expire hourly with no refresh path.
