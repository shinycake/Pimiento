...
#[test]
fn redact_secrets_for_display_scrubs_common_patterns() {
    assert_eq!(
        redact_secrets_for_display("ok Authorization: Bearer abc.def-123 end"),
        "ok Authorization: [redacted] end"
    );
    assert_eq!(
        redact_secrets_for_display("api_key=supersecret value"),
        "[redacted] value"
    );
    assert_eq!(
        redact_secrets_for_display("API-KEY: alsoSecret"),
        "[redacted]"
    );
    assert_eq!(redact_secrets_for_display("apikey=xyz"), "[redacted]");
    assert_eq!(redact_secrets_for_display("token: hush_now"), "[redacted]");
    assert_eq!(
        redact_secrets_for_display("openai sk-proj_ABC123_xyz failed"),
        "openai [redacted] failed"
    );
    let b64 = "YmFzZTY0c2VjcmV0cGF5bG9hZHdpdGhlbm91Z2hsZW5ndGg+/=";
    assert!(b64.len() >= 40);
    assert_eq!(
        redact_secrets_for_display(&format!("leak {b64} done")),
        "leak [redacted] done"
    );
    assert_eq!(
        redact_secrets_for_display("plain stderr without secrets"),
        "plain stderr without secrets"
    );
    // Truncation is applied at the status-message layer, not in the redactor.
    let long = "x".repeat(300);
    assert_eq!(redact_secrets_for_display(&long).len(), 300);
}

#[test]
fn omp_closed_status_message_redacts_and_caps() {
    assert_eq!(omp_closed_status_message(""), "OMP closed");
    assert_eq!(omp_closed_status_message("   "), "OMP closed");
    assert_eq!(
        omp_closed_status_message("token=shh"),
        "OMP closed — [redacted]"
    );
    let long = format!("error {}", "y".repeat(300));
    let msg = omp_closed_status_message(&long);
    assert!(msg.starts_with("OMP closed — "));
    let tail = msg.strip_prefix("OMP closed — ").expect("prefix");
    assert_eq!(tail.chars().count(), STDERR_STATUS_CAP);
    assert!(tail.ends_with('…'));
}

#[test]
fn supervisor_death_reason_message_crash_loop() {
    assert_eq!(
        supervisor_death_reason_message(&DeathReason::CrashLoop),
        "crash loop (3 restarts in 60s)"
    );
    assert_eq!(
        crash_loop_status_message(""),
        "OMP crash loop (3 restarts in 60s)"
    );
    assert_eq!(
        crash_loop_status_message("token=shh"),
        "OMP crash loop (3 restarts in 60s) — [redacted]"
    );
}
...
#[test]
fn filter_launcher_recents_caps_unfiltered_and_searches_all() {
    let sessions = (0u64..8)
        .map(|ix| RecentSession {
            session_file: PathBuf::from(format!("/tmp/session-{ix}.jsonl")),
            cwd: PathBuf::from(format!("/tmp/worktree-{ix}")),
            name: format!("session-{ix}"),
            last_used: ix,
        })
        .collect::<Vec<_>>();

    let unfiltered = filter_launcher_recents(&sessions, "  ");
    assert_eq!(unfiltered.len(), LAUNCHER_RECENT_VISIBLE_CAP);
    assert_eq!(unfiltered[0].name, "session-0");

    let by_name = filter_launcher_recents(&sessions, "session-7");
    assert_eq!(by_name.len(), 1);
    assert_eq!(by_name[0].name, "session-7");

    let by_path = filter_launcher_recents(&sessions, "worktree-6");
    assert_eq!(by_path.len(), 1);
    assert_eq!(by_path[0].cwd, PathBuf::from("/tmp/worktree-6"));

    assert!(filter_launcher_recents(&sessions, "nope").is_empty());
}
...
fn encode_omp_session_dir_name_matches_home_relative_layout() {
    ...
    let cwd = Path::new("/home/dev/project/Pimiento");
    assert_eq!(
...
fn parse_omp_session_header_reads_title_and_first_user() {
    let raw = concat!(
        ...
        "
",
        r#"{"type":"session","version":3,"id":"abc","timestamp":"2026-08-06T22:47:47.681Z","cwd":"/home/dev/project/Pimiento","title":"Proceed with M2 implementation"}"#,
        "
",
...
",
    ...
    assert_eq!(
        header.cwd.as_deref(),
        Some(Path::new("/home/dev/project/Pimiento"))
    );
    assert_eq!(
...
#[test]
fn omp_local_paste_dir_uses_session_under_trusted_root() {
    let root = Path::new("/tmp/pimiento-fixture/sessions");
    let session = "/tmp/pimiento-fixture/sessions/proj/abc.jsonl";
    assert_eq!(
        omp_local_paste_dir_for(Some(session), Some(root)),
        PathBuf::from("/tmp/pimiento-fixture/sessions/proj/abc/local")
    );
}

#[test]
fn omp_local_paste_dir_falls_back_without_session_or_root() {
    let fallback = std::env::temp_dir().join("omp-local").join("pimiento");
    assert_eq!(
        omp_local_paste_dir_for(None, Some(Path::new("/tmp"))),
        fallback
    );
    assert_eq!(
        omp_local_paste_dir_for(Some("/tmp/pimiento-fixture/sessions/a.jsonl"), None),
        fallback
    );
}

#[test]
fn omp_local_paste_dir_rejects_escape_outside_sessions_root() {
    let root = Path::new("/tmp/pimiento-fixture/sessions");
    let fallback = std::env::temp_dir().join("omp-local").join("pimiento");
    assert_eq!(
        omp_local_paste_dir_for(
            Some("/tmp/pimiento-fixture/sessions/../escape.jsonl"),
            Some(root)
        ),
        fallback
    );
    assert_eq!(
        omp_local_paste_dir_for(Some("/etc/passwd"), Some(root)),
        fallback
    );
    assert_eq!(
        omp_local_paste_dir_for(Some("relative/session.jsonl"), Some(root)),
        fallback
    );
}
...
mod open_url_tests {
    ...
    #[test]
    fn is_safe_open_url_accepts_http_https() {
        assert!(is_safe_open_url("https://example.com/a"));
        assert!(is_safe_open_url("http://example.com/a"));
        assert!(is_safe_open_url("HTTPS://Example.COM/oauth"));
        assert!(is_safe_open_url("HTTP://127.0.0.1:8080/callback"));
        assert!(is_safe_open_url("  https://localhost/  "));
    }

    #[test]
    fn is_safe_open_url_rejects_unsafe() {
        assert!(!is_safe_open_url(""));
        assert!(!is_safe_open_url("   "));
        assert!(!is_safe_open_url("file:///etc/passwd"));
        assert!(!is_safe_open_url("javascript:alert(1)"));
        assert!(!is_safe_open_url("ftp://x"));
        assert!(!is_safe_open_url("-evil"));
        assert!(!is_safe_open_url("smb://share/path"));
        assert!(!is_safe_open_url("custom-scheme://host"));
    }
}
