...
fn read_model_roles_map_via_omp(omp_bin: &Path) -> Result<BTreeMap<String, String>, String> {
...
pub(crate) fn assign_omp_model_role(
    omp_bin: &Path,
    role: &str,
    provider: &str,
    id: &str,
    ...
    let mut map = read_model_roles_map_via_omp(omp_bin)?;
...
fn omp_local_paste_fallback() -> PathBuf {
    std::env::temp_dir().join("omp-local").join("pimiento")
}

/// Lexically normalize an absolute path, rejecting any `..` component.
fn normalize_absolute_no_parent(path: &Path) -> Option<PathBuf> {
    if !path.is_absolute() {
        return None;
    }
...
/// True when `session_path` is constrained under `sessions_root` (canonicalize
/// when possible; otherwise reject `..` and require a lexical prefix match).
fn session_file_under_sessions_root(session_path: &Path, sessions_root: &Path) -> bool {
    ...
    let root = sessions_root
        .canonicalize()
        .unwrap_or_else(|_| sessions_root.to_path_buf());
    let Some(root) = normalize_absolute_no_parent(&root) else {
        return false;
    };

    if let Ok(canon) = session_path.canonicalize() {
        ...
        && let Ok(parent_canon) = parent.canonicalize()
    {
        ...
        return parent_canon.join(name).starts_with(&root);
    }

    // Not yet on disk — lexical only; reject `..` escapes.
    let Some(normalized) = normalize_absolute_no_parent(session_path) else {
        return false;
    };
    normalized.starts_with(&root)
}

/// Injectable paste-dir resolver (tests pass a fake sessions root).
pub(crate) fn omp_local_paste_dir_for(
    session_file: Option<&str>,
    sessions_root: Option<&Path>,
) -> PathBuf {
    let Some(session_file) = session_file.map(str::trim).filter(|s| !s.is_empty()) else {
        return omp_local_paste_fallback();
    };
    let Some(sessions_root) = sessions_root else {
        // No trusted home/sessions root — never write to an untrusted absolute path.
        return omp_local_paste_fallback();
    };

    let session_path = PathBuf::from(session_file);
    if !session_file_under_sessions_root(&session_path, sessions_root) {
        return omp_local_paste_fallback();
    }

    let base = session_file.strip_suffix(".jsonl").unwrap_or(session_file);
    PathBuf::from(base).join("local")
}

/// OMP local:// root: `{session.jsonl minus suffix}/local` when under the
/// sessions root, else temp.
pub(crate) fn omp_local_paste_dir(session_file: Option<&str>) -> PathBuf {
    omp_local_paste_dir_for(
        session_file,
        crate::app_state::omp_sessions_root().as_deref(),
    )
}
