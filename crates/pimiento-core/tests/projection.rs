...
use pimiento_core::transcript::{BOUNDED_TEXT_CAP, ToolStatus, TranscriptEntry};
...
fn text_deltas_accumulate_and_end_reconciles() {
    ...
    match &p.transcript[0] {
        TranscriptEntry::AssistantText {
            ...
            assert!(!streaming);
            assert_eq!(markdown.to_string(), "Hello, world!");
        }
        other => panic!("expected AssistantText, got {other:?}"),
    }
...
fn thinking_deltas_accumulate_and_end_reconciles() {
    ...
    match &p.transcript[0] {
        TranscriptEntry::Thinking {
            ...
            assert!(!streaming);
            assert_eq!(text.to_string(), "pondered");
        }
...
fn done_closes_streaming_rows_without_duplicating_content() {
    ...
    match &p.transcript[0] {
        TranscriptEntry::AssistantText {
            ...
            assert!(!streaming);
            assert_eq!(markdown.to_string(), "hi");
        }
        other => panic!("expected AssistantText, got {other:?}"),
    }
}
...
fn command_output_becomes_row() {
    ...
    match &p.transcript[0] {
        TranscriptEntry::CommandOutput(s) => assert_eq!(s.to_string(), "stdout tail"),
...
fn live_message_fixture() -> (Value, &'static str, String, &'static str) {
    ...
    let ls_output = concat!(
        "total 48\n",
        "drwxr-xr-x 12 dev  staff   384 Aug  7 18:38 .\n",
        "drwxr-xr-x  8 dev  staff   256 Aug  6 17:33 ..\n",
        "drwxr-xr-x  3 dev  staff    96 Aug  6 17:59 .cargo\n",
        "drwxr-xr-x 14 dev  staff   448 Aug  7 22:08 .git\n",
        "-rw-r--r--  1 dev  staff   305 Aug  6 17:59 .gitignore\n",
        "-rw-r--r--  1 dev  staff 220803 Aug  6 21:20 Cargo.lock\n",
        "-rw-r--r--  1 dev  staff  3174 Aug  6 19:10 Cargo.toml\n",
        "-rw-r--r--  1 dev  staff  3461 Aug  6 17:46 README.md\n",
        "drwxr-xr-x  5 dev  staff   160 Aug  6 17:59 crates\n",
        "drwxr-xr-x  3 dev  staff    96 Aug  7 22:08 docs\n",
        "-rw-r--r--  1 dev  staff    83 Aug  6 17:59 rust-toolchain.toml\n",
        "drwxr-xr-x  5 dev  staff   160 Aug  6 17:59 scripts\n",
        "drwxr-xr-x  8 dev  staff   256 Aug  6 18:29 target\n",
    );
...
fn hydrate_messages_replays_live_tool_and_rust_turns() {
    ...
    assert!(matches!(
        ...
        TranscriptEntry::Thinking { text, streaming: false, collapsed: true }
            if text.to_string() == "Running `ls -la` to get the directory listing."
    ));
    assert!(matches!(
        &p.transcript[2],
        TranscriptEntry::AssistantText { markdown, streaming: false }
            if markdown.to_string() == "Running `ls -la` in the workspace.\n"
    ));
    ...
    assert!(matches!(
        &p.transcript[4],
        TranscriptEntry::Thinking { text, streaming: false, collapsed: true }
            if text.to_string() == "The command completed successfully."
    ));
    assert!(matches!(
        &p.transcript[5],
        TranscriptEntry::AssistantText { markdown, streaming: false }
            if markdown.to_string() == ls_answer
    ));
    assert!(matches!(
        &p.transcript[6],
    ...
    assert!(matches!(
        &p.transcript[7],
        TranscriptEntry::Thinking { text, streaming: false, collapsed: true }
            if text.to_string() == "Preparing a Rust fenced code block example."
    ));
    assert!(matches!(
        &p.transcript[8],
        TranscriptEntry::AssistantText { markdown, streaming: false }
            if markdown.to_string() == rust_answer
    ));
}
...
fn hydrate_messages_handles_string_content_and_unmatched_results() {
    ...
    assert!(matches!(
        &p.transcript[2],
        TranscriptEntry::CommandOutput(text) if text.to_string() == "orphan output"
    ));
    assert!(matches!(
        &p.transcript[3],
...
fn duplicate_message_end_is_idempotent() {
    ...
    match &p.transcript[0] {
        TranscriptEntry::AssistantText {
            ...
            assert!(!streaming);
            assert_eq!(markdown.to_string(), "x");
        }
        other => panic!("expected AssistantText, got {other:?}"),
    }
}
...
#[test]
fn text_delta_flood_respects_bounded_text_cap() {
    let mut p = SessionProjection::new();
    apply(&mut p, json!({ "type": "message_start" }));
    apply(
        &mut p,
    ...
    while pushed < BOUNDED_TEXT_CAP * 2 {
        apply(
            &mut p,
            message_update(&json!({
                "type": "text_delta",
    ...
    match &p.transcript[0] {
        TranscriptEntry::AssistantText { markdown, .. } => {
            assert!(markdown.is_elided());
            assert!(
                markdown.to_string().len() <= BOUNDED_TEXT_CAP,
                "rendered len = {}",
                markdown.to_string().len()
            );
        }
        other => panic!("expected AssistantText, got {other:?}"),
    }
...
#[test]
fn thinking_delta_flood_respects_bounded_text_cap() {
    let mut p = SessionProjection::new();
    apply(&mut p, json!({ "type": "message_start" }));
    apply(
        &mut p,
    ...
    match &p.transcript[0] {
        TranscriptEntry::Thinking { text, .. } => {
            assert!(text.is_elided());
            assert!(
                text.to_string().len() <= BOUNDED_TEXT_CAP,
                "rendered len = {}",
                text.to_string().len()
            );
        }
...
#[test]
fn text_end_oversized_content_cannot_bypass_cap() {
    let mut p = SessionProjection::new();
    apply(&mut p, json!({ "type": "message_start" }));
    apply(
        &mut p,
        message_update(&json!({
            "type": "text_delta", "contentIndex": 0, "delta": "seed", "partial": {}
        })),
    );
    let oversized = "X".repeat(BOUNDED_TEXT_CAP * 2);
    apply(
        &mut p,
        message_update(&json!({
            "type": "text_end",
    ...
    match &p.transcript[0] {
        TranscriptEntry::AssistantText {
            ...
            assert!(!streaming);
            assert!(markdown.is_elided());
            assert!(markdown.to_string().len() <= BOUNDED_TEXT_CAP);
            // Must not retain the small delta prefix — end replaces, not appends.
            assert!(!markdown.to_string().starts_with("seed"));
        }
        other => panic!("expected AssistantText, got {other:?}"),
    }
}
...
#[test]
fn thinking_end_oversized_content_cannot_bypass_cap() {
    let mut p = SessionProjection::new();
    apply(&mut p, json!({ "type": "message_start" }));
    apply(
        &mut p,
        message_update(&json!({
            "type": "thinking_delta", "contentIndex": 0, "delta": "seed", "partial": {}
        })),
    );
    let oversized = "Y".repeat(BOUNDED_TEXT_CAP * 2);
    apply(
        &mut p,
        message_update(&json!({
            "type": "thinking_end",
    ...
    match &p.transcript[0] {
        TranscriptEntry::Thinking {
            ...
            assert!(!streaming);
            assert!(text.is_elided());
            assert!(text.to_string().len() <= BOUNDED_TEXT_CAP);
            assert!(!text.to_string().starts_with("seed"));
        }
        other => panic!("expected Thinking, got {other:?}"),
    }
}
...
#[test]
fn command_output_oversized_respects_cap() {
    let oversized = "Z".repeat(BOUNDED_TEXT_CAP * 2);
    let mut p = SessionProjection::new();
    apply(
        &mut p,
        json!({ "type": "command_output", "output": oversized }),
    );
    match &p.transcript[0] {
