...
//!
//! [`BoundedText`] caps stored assistant markdown, thinking text, tool output,
//! and command output at [`BOUNDED_TEXT_CAP`] rendered bytes — retaining a
//! deterministic head and tail joined by a stable elision marker of the form
//! `"\n…[N bytes elided]\n"`. All append operations preserve UTF-8 boundaries
//! and never require more than a bounded amount of transient allocation on top
//! of the caller-owned chunk (§5.4, SH-5).
...
///
/// 512 KiB — the storage budget This bounds the
...
impl BoundedText {
    ...
    /// Replaces the buffer with `s`, applying the same elision rules as
    /// [`push_str`]. Prefer this (or `clear` + `push_str`) over cloning a raw
    /// wire string when reconciling `*_end` frames.
...
impl From<&str> for BoundedText {
    fn from(s: &str) -> Self {
        Self::from_text(s)
    }
}

impl From<String> for BoundedText {
...
/// Status of a tool invocation on the transcript.
...
/// Minimal markdown representation for M2 — bounded raw source string.
///
/// SH-era rendering will replace this with a parsed IR; the wire text is
/// preserved here (under [`BOUNDED_TEXT_CAP`]) so replay is faithful and later
/// parsers are drop-in.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct Markdown(pub BoundedText);

impl Markdown {
    #[must_use]
    pub fn new(s: impl AsRef<str>) -> Self {
        Self(BoundedText::from_text(s.as_ref()))
    }

    /// Appends a UTF-8 chunk under the shared [`BOUNDED_TEXT_CAP`].
    pub fn push_str(&mut self, chunk: &str) {
        self.0.push_str(chunk);
    }

    /// Replaces contents from authoritative wire text (`text_end`). Never
    /// bypasses the cap via a raw `clone_from`.
    pub fn set_text(&mut self, s: &str) {
        self.0.set_text(s);
    }
    ...
    /// Borrowed view of the retained head. When not elided this is the full
    /// text; when elided prefer [`Display`] / [`Self::render`] for the capped
    /// form including the elision marker and tail.
    #[must_use]
    pub fn as_str(&self) -> &str {
        self.0.head()
    }

    /// Renders `head + marker + tail` (same as [`Display`]'s owned form via
    /// [`BoundedText::render`]).
    #[must_use]
    pub fn render(&self) -> String {
        self.0.render()
...
/// One row in the transcript. `Unknown` MUST always render.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
