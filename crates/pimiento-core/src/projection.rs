...
//! replay tests and for GPUI to project onto. See `PLAN.md` §5.4 and §6-M2.
//!
//! Invariants:
//!
...
use crate::transcript::{CompactionPhase, Markdown, ToolCall, ToolStatus, TranscriptEntry};
        ...
        match &mu.assistant_message_event.kind {
            ...
            AssistantMessageEventKind::TextEnd {
                ...
                }) = self.transcript.get_mut(i)
                {
                    ...
                    // append (which would duplicate the deltas).
                    markdown.0.clone_from(content);
                    *streaming = false;
                }
            }
