...
`dist/Pimiento.app`. Icon masters live under `assets/` in the Zed-style
pair `app-icon.png` (512) + `app-icon@2x.png` (1024): full-bleed square
PNGs with **no** pre-baked rounded corners (Apple HIG — the system applies
the macOS squircle mask). The packager builds `Contents/Resources/Pimiento.icns`
via `iconutil` and sets `CFBundleIconFile` to `Pimiento` (no extension).
Re-running the script replaces the existing bundle.
...
Windows packaging is optional and deferred; no support claim is
