...
- **Chat gutters + growing composer (2026-08-11):** GPUI `list` only applies vertical padding to item origins — horizontal `.px_*` on the list left prose flush to the column edge. Each transcript row now carries `transcript_gutter_x()` (24px) plus a 16px post-rail content pad. Composer chrome uses `min_h(40)` (not fixed height) so `InputState::auto_grow(1, 8)` can expand; pending image attachments show a 56px Cover thumbnail preview (clipboard paste / Attach / DnD already wire `PendingAttachment::Image`).
- **Composer paste intercept (2026-08-11):** GPUI dispatches Input keybindings (including `Paste`) *before* `capture_key_down`. SessionView now uses `cx.intercept_keystrokes` so Ctrl/Cmd+V can promote clipboard images and existing file paths to pending attachments instead of inserting raw text into the Input.
