...
Wire extraction baseline: locally installed `omp/17.2.10`. Compatibility has also been exercised against **omp 17.2.11**; the documented tested range is **17.2.10–17.2.11**.

Canonical sources inspected against the locally installed OMP **17.2.10** package
(`@oh-my-pi/pi-coding-agent`, `@oh-my-pi/pi-agent-core`, `@oh-my-pi/pi-ai`),
especially `src/modes/rpc/rpc-types.ts` and `src/modes/rpc/rpc-frame.ts`.
...
## Model wire shapes (OMP 17.2.10) — 2026-08-07

- `get_state` returns the session state object directly as `data` (not wrapped in `{state:…}`).
- `state.model` is a **Model object** `{ provider, id, … }`, not a string. Pimiento formats it as `provider/id` for display and `set_model`.
- `get_available_models` → `{ models: Model[] }`.
- `set_model { provider, modelId }` success `data` is the Model object.
- `model_changed` is a **bare** event `{ type: "model_changed" }` with **no** model payload. Clients must refresh via `get_state` or trust the preceding `set_model` response.
...
- Popups and overlays must use gpui-component primitives so placement remains backend-owned. Wayland and X11 popup placement remain platform-sensitive; verify on both Linux backends.
...
- Composer band uses `shadow_lg` as the sole docked elevation exception.
...
- **Session rail layout (2026-08-10):** top chrome (Workspace… / hide) is hairline-separated from the list. Each workspace is a muted secondary plate (folder icon + basename + non-idle rollup pill + `+`) with a `~/…` truncated path and session count; sessions nest underneath with a reserved attention-dot column for alignment. Inter-workspace `Separator`s emphasize group boundaries. Collapsed rail is icon-only (⌘B stays in the tooltip).
...
- Wave 0 documentation now reflects image attachments as shipped, records the **omp 17.2.10–17.2.11** tested range, and separates feature completion from still-open platform QA.
- Wave E adds a minimal Linux personal release tarball path. AppImage, `.deb`, signing, and Windows remain deferred; Pimiento still neither bundles nor installs `omp`.
- Motion durations and reduced-motion behavior should be verified manually on each release (see `docs/manual-qa-checklist.md`).
- This entry summarizes the Quiet Pepper execute integration change set; it does not assert completed platform QA.
...
## Native desktop menus and lifecycle — 2026-08-09

- The pinned Apache-2.0 GPUI reference is `crates/gpui/examples/set_menus.rs`: call `App::set_menus` before opening a window, use `MenuItem::os_submenu(..., SystemMenuType::Services)` for the macOS Services menu, and register application-wide actions with `App::on_action`.
- The first top-level menu is explicitly named **Pimiento**. This gives the unbundled debug executable a product-named macOS application menu without depending on release packaging.
